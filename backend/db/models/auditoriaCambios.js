...
/**
 * ========================================
 * FUNCIONES PARA CAMBIOS TOTALES (PREDIOS + GEOGRÁFICOS)
 * ========================================
 */

/**
 * Obtiene todos los cambios desde la vista unificada view_cambios_total
 */
const obtenerCambiosTotal = async (params) => {
  const {
    fecha_inicio,
    fecha_fin,
    clave_catastral,
  ...
  let baseQuery = `
    SELECT 
      ...
      indice_actividad, 
      fuente
    FROM actividad.view_cambios_total 
    WHERE 1=1
  `;
  ...
  if (fecha_inicio && fecha_fin) {
    queryParams.push(fecha_inicio + ' 00:00:00', fecha_fin + ' 23:59:59');
    baseQuery += ` AND fecha_actividad BETWEEN $${queryParams.length - 1} AND $${queryParams.length}`;
    console.log(`🕐 Filtro fechas totales: ${fecha_inicio} 00:00:00 hasta ${fecha_fin} 23:59:59`);
  } else if (fecha_inicio) {
    queryParams.push(fecha_inicio + ' 00:00:00');
    baseQuery += ` AND fecha_actividad >= $${queryParams.length}`;
    console.log(`🕐 Filtro fecha inicio total: ${fecha_inicio} 00:00:00`);
  } else if (fecha_fin) {
    queryParams.push(fecha_fin + ' 23:59:59');
    baseQuery += ` AND fecha_actividad <= $${queryParams.length}`;
    console.log(`🕐 Filtro fecha fin total: ${fecha_fin} 23:59:59`);
  }
  ...
  try {
    ...
    // Validar que el resultado existe
    if (!countResult || !countResult.rows || countResult.rows.length === 0) {
      console.log('⚠️ No se encontraron resultados en la consulta de conteo total');
      return {
        items: [],
        pagination: {
          total: 0,
    ...
    console.log('✅ Cambios totales encontrados:', result.rows?.length || 0, 'registros');
    ...
    console.error('❌ Error al consultar cambios totales:', err);
    console.error('❌ Stack:', err.stack);
    console.error('❌ Query base:', baseQuery);
    console.error('❌ Parámetros:', queryParams);
...
/**
 * Obtiene las acciones disponibles para filtrado desde la vista total
 */
const obtenerAccionesTotalesDisponibles = async () => {
  try {
    const query = `
      SELECT DISTINCT accion_actividad 
      FROM actividad.view_cambios_total 
    ...
    console.error('❌ Error al obtener acciones totales disponibles:', err);
    console.error('❌ Stack:', err.stack);
    return [];
  }
};
...
/**
 * Obtiene estadísticas básicas de todos los cambios por período
 */
const obtenerEstadisticasCambiosTotal = async (fecha_inicio, fecha_fin, id_tipo_predio = null) => {
  try {
    let query = `
      SELECT 
        accion_actividad,
    ...
    const params = [fecha_inicio + ' 00:00:00', fecha_fin + ' 23:59:59'];
    console.log(`📊 Estadísticas totales: ${fecha_inicio} 00:00:00 hasta ${fecha_fin} 23:59:59, tipo: ${id_tipo_predio || 'todos'}`);
    ...
    console.error('❌ Error al obtener estadísticas de cambios totales:', err);
    console.error('❌ Stack:', err.stack);
    console.error('❌ Query:', query);
    console.error('❌ Parámetros:', params);
    return [];
...
module.exports = {
  ...
  obtenerCambiosTotal,
  obtenerAccionesTotalesDisponibles,
  obtenerEstadisticasCambiosTotal
};