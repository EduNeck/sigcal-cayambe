  ...
  <v-container class="container">
    <v-row justify="center">
      <v-col cols="12" class="text-center">
        <h2 class="titulo-pantalla">Listado de Bloques No Digitados</h2>
      </v-col>
    ...
    <v-row>
      <v-col cols="12">
        ...
        >
          <template v-slot:top>
            <v-toolbar flat>
              <v-toolbar-title>Bloques No Digitados</v-toolbar-title>
...
export default {
  ...
  methods: {
    async cargaBloques() {
      try {
        const response = await axios.get(`${API_BASE_URL}/vcontrol_bloques_no_digitados?tipo_zona=1`);
