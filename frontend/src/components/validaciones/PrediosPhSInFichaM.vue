  ...
  <v-container class="container">
    <v-row justify="center">
      <v-col cols="12" class="text-center">
        <h2 class="titulo-pantalla">Listado de Predios PH sin Ficha Maestra</h2>
      </v-col>
    ...
    <v-row>
      <v-col cols="12">
        ...
        >
          <template v-slot:top>
            <v-toolbar flat>
              <v-toolbar-title>Predios PH sin Ficha Maestra</v-toolbar-title>
          ...
          <template v-slot:[`item.clave_ph`]="{ item }">
            {{ item.clave_ph }}
          </template>
          <template v-slot:[`item.tipo_ph`]="{ item }">
            {{ item.tipo_ph }}
          </template>
          ...
          <template v-slot:[`item.clave_maestra`]="{ item }">
            {{ item.clave_maestra }}
          </template>
          <template v-slot:[`item.tipo_fm`]="{ item }">
            {{ item.tipo_fm }}
          </template>
...
export default {
  name: 'PrediosPhSInFichaM',
  data() {
    return {
      search: '',
      headers: [        
        { title: 'Clave PH', value: 'clave_ph' },
        { title: 'Tipo PH', value: 'tipo_ph' },
        { title: 'Bloque', value: 'bloque' },
        { title: 'Piso', value: 'piso' },
        { title: 'Unidad', value: 'unidad' },
        { title: 'Clave Maestra', value: 'clave_maestra' },
        { title: 'Tipo FM', value: 'tipo_fm' },
  ...
  methods: {
    async cargaPredios() {
      try {
        const response = await axios.get(`${API_BASE_URL}/vcontrol_ph_sin_fm?id_tipo_predio=1`);
