  ...
  <v-container class="container">
    <v-row justify="center">
      <v-col cols="12" class="text-center">
        <h2 class="titulo-pantalla">Listado de Predios sin Servicios</h2>
      </v-col>
    ...
    <v-row>
      <v-col cols="12">
        ...
        >
          <template v-slot:top>
            <v-toolbar flat>
              <v-toolbar-title>Predios sin Servicios</v-toolbar-title>
...
export default {
  ...
  methods: {
    async cargaPredios() {
      try {
        const response = await axios.get(`${API_BASE_URL}/vcontrol_predios_sin_servicios?id_tipo_predio=1`);
