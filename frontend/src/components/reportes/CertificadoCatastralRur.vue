    ...
    <div id="certificado" class="a4-container">
      ...
      <v-card>
        <v-card-title class="block-title fondo-sub">
          <p class="text-center">CERTIFICADO DE AVALÚS Y CATASTROS RURALES N°: {{ form.numero_serie }}</p>
        </v-card-title>
  ...
  methods: {
    ...
    async insertaCertificadoCatastral() {
      try {
        ...
        const response = await axios.post(`${API_BASE_URL}/insertaCertificadoCatastral`, this.form);
